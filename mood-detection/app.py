...
def get_mfccs(audio, limit):
  y, sr = librosa.load(audio, sr=44100)
  a = librosa.feature.mfcc(y, sr=44100, n_mfcc = 40)
  if a.shape[1] > limit:
    mfccs = a[:,:limit]
  elif a.shape[1] < limit:
    mfccs = np.zeros((40, limit))
    mfccs[:, :a.shape[1]] = a
  else: 
    mfccs = a
  return mfccs.T
...
def main():
    ...
    if choice == "Upload audio":
        ...
        if audio_file is not None:
            ...
            print(f'\n\nmodel shape: {model.input_shape[-1]}\n\n')
            mfccs = get_mfccs(path, 236)
        ...
        st.subheader("About")
        st.info("vatsal2473@gmail.com")
        st.info("chakridevireddy69@gmail.com")
        st.info("prince.singh.iiitl@gmail.com")
        st.info("pbaderia0@gmail.com")
